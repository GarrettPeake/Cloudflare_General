                ...
                <div className='flex overflow-y-hidden rounded-lg overflow-x-auto mb-1 h-full'>
                    {
                        this.props.imgs.map((element, index) => {
                            return <button key={index} onClick={this.blowup} className=' flex-none w-36 h-36 mr-2 mb-1 rounded-lg'><img alt='Post media' name={element} className='h-full w-full rounded-lg' src={element}/></button>
                        })
                    }
                </div>
                { this.state.blowup ?
                    <button onClick={this.blowup} name={null} className='fixed h-full w-full z-50 bg-opacity-60 bg-gray-500 left-0 top-0 flex items-center justify-center'>
                        <img alt='Post media' className='h-5/6 rounded-lg' src={this.state.blowup}/>
                    </button>
                : null}
            </div>
        )
